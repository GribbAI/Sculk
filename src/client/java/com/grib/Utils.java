...
public class Utils {
    ...
    public static double[] worldToCamera(
            double dx,
            double dy,
            double dz,
            double yawRad,
    ...
    public static void drawTextWithShadow(
        ...
        ctx.drawText(renderer, text, x + 1, y + 1, 0x80000000, true);
        ctx.drawText(renderer, text, x, y, color, true);
    }
    ...
    public static void drawRectOutline(
        ...
        fillRect(ctx, left, top, right, top + 1, color);
        fillRect(ctx, left, bottom - 1, right, bottom, color);
        fillRect(ctx, left, top, left + 1, bottom, color);
        fillRect(ctx, right - 1, top, right, bottom, color);
    }
    ...
    public static void drawDashedThickLine(
        ...
        double stepX = dx / (double) len;
        double stepY = dy / (double) len;

        int gap = 4;

        for (int i = 0; i <= len && i < 2000; i++) {
            boolean draw = (i % (gap * 2)) < gap;
            if (!draw) continue;

            int cx = (int) Math.round(x0 + stepX * i);
            int cy = (int) Math.round(y0 + stepY * i);
    ...
    public static int blendAlpha(int color, float alphaFactor) {
        int a = (color >> 24) & 0xFF;
        int r = (color >> 16) & 0xFF;
        int g = (color >> 8) & 0xFF;
        int b = color & 0xFF;
